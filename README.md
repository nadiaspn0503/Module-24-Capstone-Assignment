...
- The first content-based recocmmendation system is a random forest classifier. This model took 9.9568 seconds to train. The precision score is 0.5769. The recall score is 0.6889. With an okay training time, this model gives more recommendations that aren't too relevant considering the recall is higher than the precision.

- The second content-based recocmmendation system is an ANNOY index. This model took 63.1538 seconds to train. The precision score is 0.0019. The recall score is 0.0093. With a significantly longer training time than the other content-based models and extreely low precision and recall scores, this model performed poorly.

- The third content-based recommendation system is an HNSW model. This model took 14.7216 seconds to train. The precision score is 0.5379. The recall score is 0.5385. Although this model took longer to train and has slightly lower precision and recall scores than the random forests recommendation system, the HNSW model's precision and recall scores are more balanced with each other. Therefore, this is the content-based model that will be used for the hybrid recommendation system.
...
- Five collaborative-based recommendation systems are trained and evaluated together. These models include KNNBasic, SVD, NMF, CoClustering, and SlopeOne. KNNBasic has a training time of 0.9771 seconds, a mean precision score of 0.7733, and a mean recall score of 0.2838. SVD has a training time of 0.5358 seconds, a mean precision score of 0.8216, and a mean recall score of 0.3040. NMF has a training time of 3.2977 seconds, a mean precision score of 0.7941, and a mean recall score of 0.2922. CoClustering has a training time of 3.1246 seconds, a mean precision score of 0.8092, and a mean recall score of 0.2983. SlopeOne has a training time of 0.0000 seconds, a mean precision score of 0.8185, and a mean recall score of 0.3029. After SVD, SlopeOne has the best mean precision and mean recall scores. SlopeOne also has no paramters to tune and has a short training time. Therefore, the SlopeOne model will be used for the hybrid recommendation system. It should be noted that the SlopeOne model was redone on its own to provide ease when creating and comparing the hybrid recommendation systems.
...
- The first hybrid recocmmendation system is a weighted recommendation system. This model took 0.4998 seconds to train. The precision score is 0.3593. The recall score is 0.5400.

- The second hybrid recocmmendation system is a switching recommendation system. This model took 8.5354 seconds to train. The precision score is 0.6300. The recall score is 0.6300.

- The third hybrid recocmmendation system is a mixed recommendation system. This model took 8.9169 seconds to train. The precision score is 0.5422. The recall score is 0.5443. 

- The fourth hybrid recocmmendation system is a cascade recommendation system. This model took 8.8215 seconds to train. The precision score is 0.5513. The recall score is 0.5539.

- The fifth hybrid recocmmendation system is a feature recommendation system. This model took 6.0842 seconds to train. The precision score is 0.0010. The recall score is 0.0050. 

### Findings

- As a reminder, the goal of creating these models is to help Goodreads establish a recommendation system that can accurately predict books that each user will enjoy. This will help Goodreads retain their current customers and gain more customers as they will find the recommendations they receive to be accurate to their interests. Goodreads will make more money and users will have an enjoyable experience reading and finding books. After some exploratory data analysis, it is shown that most of the books that were reviewed had high average scores. This is important as Goodreads shows and considers the average score when making recommendations, especially collaborative-based ones. However, individual ratings are also important as users may not always like popular books, which is where content-based recommendations come in. Overall, the hybrid recommendation systems yielded the best outcomes in terms of training times, precisions scores, and recall scores.
  
- Of the five hybrid recommendation systems that were created and compared, the switching hybrid model seems to have performed the best. Although this model's training time is within the top three highest training times of the hybrid recommendation systems, it only took a little over 8 seconds. The main contributions to this model being deemed the best are its precision and recall scores. This model has the highest and most balanced scores. The precision and recall scores are also identifcal meaning the users are recising a higher proportion of recommendations, and of those recommendations, a higher proportion of them are relevant compared to the other hybrid recommendation systems.
  
- Considering the users in this dataset seem to have given a high amount of reviews (gives more data) and the books that are reviewed are mostly popular ones, a switching hybrid recommendation system is great as it switches from the content-based model for new users to avoid only recommending irrelevant yet popular books to the collaborative model as new data is added for the model to see where is the collective group does the user fall.

### Next Steps

- Further data collection, data cleaning predictive modeling, and hyperparameter tuning are best. Data collection would be the most important. This is because most of the users in the dataset have reviewed many books. The average number of books read/reviewed given by the top reviewers was 194.32 books/reviews. However, new users with little data cannot rely as much on content-based filtering as much as older users. Data with books that are not only trendy is also important. Not everyone enjoys reading trendy books all the time. Some people want new and niche book recommendations. The current dataset has a bias toward popular books. It would be beneficial to include untrendy books in the dataset to help strengthen the content-based aspect of the recommendation system so that people aren't receiving the same book recommendations over and over without uniqueness in their own algorithm. With new data will also need data cleaning. However, it should be mentioned again that the dataset used in the project was already put together and did not come directly from the Goodreads website as the original dataset was too big to download and open on my computer. Therefore, a dataset that isn't so computationally expensive would also be helpful. There are many types of hybrid recommendation systems, therefore, exploring as many as possible would be helpful. The most important next step is hyperparamter tuning. It should be noted that the data sizes and paramters used in these model were chosen to optimize the model for time. This does not neccassarliy mean that the swithing model is the best overall. However, the goodreads dataset is large which caused the models to be computationally expensive. Recreating these models with smaller groups of the data, like hoosing one genre of books, may be more beneficial and less computationally expensive. Another option is to choose other content-based and collaborative-based recommendation systems to use in the hybrid recommendation system.
